...
const statsData = {
  ...
  moduleStats: [
    { name: 'Hiragana', progress: 85, cardsReviewed: 450, accuracy: 90, timeSpent: '15 hours' },
    { name: 'Katakana', progress: 70, cardsReviewed: 320, accuracy: 85, timeSpent: '12 hours' },
    { name: 'Numbers', progress: 95, cardsReviewed: 280, accuracy: 92, timeSpent: '8 hours' },
    { name: 'Time', progress: 60, cardsReviewed: 200, accuracy: 82, timeSpent: '10 hours' }
  ],
  monthlyProgress: [
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
              <Card className="p-6 text-center bg-background/10 backdrop-blur-sm border-primary-foreground/20">
                ...
                <h3 className="text-2xl font-bold text-primary-foreground">{statsData.overallStats.totalStudyTime}</h3>
                <p className="text-primary-foreground/80">Total Study Time</p>
              </Card>
              <Card className="p-6 text-center bg-background/10 backdrop-blur-sm border-primary-foreground/20">
                <BookOpen className="w-8 h-8 text-green-400 mx-auto mb-2" />
                <h3 className="text-2xl font-bold text-primary-foreground">{statsData.overallStats.totalCardsReviewed}</h3>
                <p className="text-primary-foreground/80">Cards Reviewed</p>
              </Card>
              <Card className="p-6 text-center bg-background/10 backdrop-blur-sm border-primary-foreground/20">
                <Target className="w-8 h-8 text-blue-400 mx-auto mb-2" />
                <h3 className="text-2xl font-bold text-primary-foreground">{statsData.overallStats.averageAccuracy}%</h3>
                <p className="text-primary-foreground/80">Average Accuracy</p>
              </Card>
              <Card className="p-6 text-center bg-background/10 backdrop-blur-sm border-primary-foreground/20">
                <Trophy className="w-8 h-8 text-orange-400 mx-auto mb-2" />
                <h3 className="text-2xl font-bold text-primary-foreground">{statsData.overallStats.longestStreak}</h3>
                <p className="text-primary-foreground/80">Longest Streak</p>
              </Card>
              <Card className="p-6 text-center bg-background/10 backdrop-blur-sm border-primary-foreground/20">
                <TrendingUp className="w-8 h-8 text-purple-400 mx-auto mb-2" />
                <h3 className="text-2xl font-bold text-primary-foreground">{statsData.overallStats.totalWordsLearned}</h3>
                ...
                {statsData.moduleStats.map((module, index) => (
                  <div key={index} className="p-4 bg-background/5 rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="font-semibold text-primary-foreground">{module.name}</h4>
                      <span className="text-sm text-primary-foreground/80">{module.progress}% Complete</span>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        <span className="text-primary-foreground/80">Cards:</span>
                        <span className="font-semibold ml-2 text-primary-foreground">{module.cardsReviewed}</span>
                      </div>
                      <div>
                        <span className="text-primary-foreground/80">Accuracy:</span>
                        <span className="font-semibold ml-2 text-green-400">{module.accuracy}%</span>
                      </div>
                      <div>
                        <span className="text-primary-foreground/80">Time:</span>
                        <span className="font-semibold ml-2 text-primary-foreground">{module.timeSpent}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                {statsData.monthlyProgress.map((month, index) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-background/5 rounded-lg">
                    <span className="font-semibold text-primary-foreground">{month.month}</span>
                    <div className="flex items-center space-x-4">
                      <span className="text-sm text-primary-foreground/80">{month.cards} cards</span>
                      <span className="text-sm font-semibold text-green-400">{month.accuracy}%</span>
                    </div>
                  </div>
                ))}
              </div>
