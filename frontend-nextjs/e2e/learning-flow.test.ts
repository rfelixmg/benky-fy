...
test.describe('Core Learning Flows', () => {
  ...
  test('verify landing page colors module card', async ({ page }) => {
    ...
    await expect(colorsCard.locator('p')).toHaveText('Learn Japanese color words with visual aids');
  ...
  test('verify colors module content and functionality', async ({ page }) => {
    ...
    for (const set of practiceSets) {
      await expect(page.locator(`[data-testid="practice-set-${set}"]`)).toBeVisible();
    }
    ...
    await expect(page.locator('[data-testid="progress-tracker"]')).toContainText('1');
  });
